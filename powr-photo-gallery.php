<?php
    ...
    Plugin URI: https://www.powr.io/plugins/photo-gallery
    ...
    */
    ...
    class Powr_Photo_Gallery extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Photo Gallery.
        </p>
        <p>
    ...
    function powr_photo_gallery_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-photo-gallery' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-photo-gallery' label='$label'></div>";
      }else{
      	"<div class='powr-photo-gallery'></div>";
      }
    }
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_photo_gallery_menu' );
    function powr_photo_gallery_menu() {
      add_menu_page( 'POWr Photo Gallery', 'POWr Photo Gallery', 'manage_options', 'powr-photo-gallery-settings', 'powr_photo_gallery_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_photo_gallery_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/photo-gallery';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/photo-gallery';
      }
    ...
    register_activation_hook(__FILE__, 'powr_photo_gallery_plugin_activate');
      add_action('admin_init', 'powr_photo_gallery_plugin_redirect');

      function powr_photo_gallery_plugin_activate() {
      add_option('powr_photo_gallery_plugin_do_activation_redirect', true);
      }
      ...
      function powr_photo_gallery_plugin_redirect() {
      if (get_option('powr_photo_gallery_plugin_do_activation_redirect', false)) {
          delete_option('powr_photo_gallery_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-photo-gallery-settings&' );
          }
       }
      }
